...
<ul>
  ...
  <li>Improvement: Issue with replicating exact schedules. This is likely due to the fact that while inputs are required to be unique, there is nothing ensuring unique outcomes. Due to the -15/+15/-30/+30 calibration if a day does not work outright, multiple requests that have a close range could both be 'calibrated' to the same time on different scheduling attempts, and create similar outcomes on different tree explorations. Recommendation: find a time/cost efficient way to ensure no duplication of schedules occur due to calibration</li>
